...
export default function AdminReportReview() {
  const { user } = useAuth();
  const [pendingReports, setPendingReports] = useState<ExamOfficerReport[]>([]);
  const [allReports, setAllReports] = useState<ExamOfficerReport[]>([]);
  const [viewingReport, setViewingReport] = useState<ExamOfficerReport | null>(null);
  const [reviewingReport, setReviewingReport] = useState<ExamOfficerReport | null>(null);
  ...
  const handleApprove = (reportId: string, comments?: string) => {
    const updated = reportStorage.approveReport(reportId, user!.id, comments);
    if (updated) {
      loadReports();
  ...
  const handleReject = (reportId: string, comments: string) => {
    ...
    const updated = reportStorage.rejectReport(reportId, user!.id, comments);
    if (updated) {
      loadReports();
      setReviewingReport(null);
      setReviewComments('');
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900">Report Review Center</h2>
        <p className="text-gray-600">Review and approve exam officer reports</p>
      </div>
          ...
          {currentReports.length === 0 ? (
            <div className="text-center py-8">
              ...
              <h3 className="text-lg font-medium text-gray-900">
                {activeTab === 'pending' ? 'No Pending Reports' : 'No Reports Available'}
              </h3>
